...
The three codes below generalizes the small pieces included in this document to create the daily county-level weather data in 2019 and 2020 for all the counties in the Contiguous US. Constructed weather variables include precipitation, and degree days above 0, 5, 8, 10, 12, 15, 20, 25, 29, 30, 31, 32, 33 and 34 degree Celsius. 



```{r}
list.files("Code/", pattern = "^1")
...
## Load PRISM data
```{r}
file_list <- list.files("Data/prism/",
                        full.names = T,
...
```{r,  class.output="scroll"}
nlcd_R <- raster("Data/nlcd_2019_land_cover_l48_20210604/nlcd_2019_land_cover_l48_20210604.img")
...
```{r, results='hide'}
prism_P$nlcd <- exactextractr::exact_extract(nlcd_R, prism_P)
```
...
# Calculating county-level temperatures 
```{r, results='hide'}
